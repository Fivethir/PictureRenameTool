    ...
    def load_device_count(self):
        ...
        if os.path.exists(log_path):
            with open(log_path, 'r', encoding='utf-8') as f:
                for line in f:
                    if line.startswith("[设备统计]"):
                        try:
                            parts = line.strip().split(": ")[1].split(", ")
                            for part in parts:
                                if ":" in part:
                                    dev, num = part.split(":", 1)
                                    if dev in count:
                                        count[dev] = int(num)
                        except:
                            continue
    ...
    def rename_copy(self):
        ...
        try:
            ...
            if self.device_name in self.device_count:
                self.device_count[self.device_name] += 1
            ...
            # 原有日志
            log_old = f"[{timestamp}] 生成文件：{new_name}"
            # 新增“领取”日志
            if remark:
                log_new = f"[{timestamp}] 领取{self.device_name}_{remark}_{quantity_str}个"
            else:
                log_new = f"[{timestamp}] 领取{self.device_name}_{quantity_str}个"
    ...
    def view_log(self):
        ...
        if password == "admin5678":
            ...
            try:
                ...
                # 兜底：显示错误
                messagebox.showerror("错误", "无法打开日志文件，请手动查看同目录下的 rename_log.txt")
        else:
